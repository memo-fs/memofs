...
>
> **Critical scoping note (user directive, 2026-06-20):** **Memo FS Cloud, the
...
> (b) the **deleted `memofs-adapter-upstash`** package; (c) genuine missing
...
> Memo FS Cloud and can't read your local disk) is a **true product claim — it
...
Code reality: `packages/memofs/src/memofs/types.ts` (the constructor `mode`
...
| path | verdict | rule |
|---|---|---|
| `api/memofs/memofs.md:23` | mode union still lists `"cloud"`. → drop it; union becomes `local \| hybrid \| memory`. | R1 |
| `api/memofs/memofs.md:26-27` | `readPolicy`/`writePolicy` still list `"cloud-only"` (and `"cloud-first"` if removed). → drop the removed rows. | R1 |
| `api/memofs/memofs.md:221` | example uses `mode: "cloud"`. → replace with a `hybrid` example or a sync-client example. | R1 |
| `packages/memofs/configuration.md:30-46` | "## Cloud" section with `mode: "cloud"` + `cloud: { baseUrl, apiKey }`. → replace the *constructor* example with the sync-client / hosted-endpoint path; keep the cloud as a shipping surface. | R1 |
| `packages/memofs/configuration.md:82` | config table still enumerates `"cloud"` under `runtime`. → drop. | R1 |
| `packages/memofs/configuration.md:91-92` | `hybrid.readPolicy` / `writePolicy` still list `"cloud-only"` (+ `"cloud-first"`). → drop removed rows. | R1 |
| `packages/memofs/configuration.md:171` | config.json example has a `"cloud": {}` block keyed off the old mode. → update to match the sync-client config shape. | R1 |
| `packages/memofs/client.md:45` | constructor example `mode: "cloud"`. → replace with sync-client / hosted framing. | R1 |
| `packages/memofs/client.md:109` | policy table row `cloud-only`. → drop the row. | R1 |
| `packages/memofs/cloud-client.md:15,21,177` | "Set `mode: "cloud"`" framing. → reframe around the cloud client as the **sync transport** (`health`/`readiness`/`sync`), not a runtime mode. Cloud client stays; the *mode flag* goes. | R1 |
| `packages/memofs/concepts.md:52` | "Hybrid runtime" gloss lists `cloud-only` in the policy set. → drop the removed value. | R1 |
...
> Memo FS Cloud, can't read local disk) is a true product claim and stays.
...
### S2-Q1 / ADR 0007 — AI SDK extracted; `MemofsAiRuntime` renamed

Code reality: the 14 `src/ai-sdk/*` files moved to
`packages/memofs-adapter-ai-sdk/`; the interface renamed
`MemofsAiRuntime` → `MemofsMemoryRuntime` (now in
`packages/memofs/src/ai-runtime/`); the `ai` peer dep dropped from core.
**These docs still name the old interface and/or place the integration in core.**

| path | verdict | rule |
|---|---|---|
| `packages/memofs/ai-sdk/index.md:80,83` | names `MemofsAiRuntime`; references `mode: "cloud"`. → `MemofsMemoryRuntime`; integration now in `@memofs/adapter-ai-sdk` — import from there. | R1/R2 |
| `packages/memofs/ai-sdk/tools.md:9,20,51` | names `MemofsAiRuntime` (×3). → `MemofsMemoryRuntime`; update imports to the adapter package. | R1/R2 |
| `api/memofs/ai-sdk.md:93` | example `mode: "cloud"` (D4). → remove/replace; confirm this API page is generated from the **adapter's** exports, not core's (core no longer exports these). | R1/R2 |

### Q6 6a — `memofs-adapter-upstash` deleted

Code reality: package deleted; lockfile + 9 ref files pruned.

| path | verdict | rule |
|---|---|---|
| `api/memofs/recall.md:28` | links to `[Upstash Vector adapter](./vector-adapters)` — also a **dead link** to a page that doesn't exist (see Missing #1). → drop the upstash mention. | R1 |
| `packages/memofs/installation.md:42` | "Upstash Vector recall" install row for `@upstash/vector`. → remove the row. | R1 |
...
|---|---|---|---|
| `api/memofs/vector-adapters.md` | **sidebar links to it (`config/sidebar.mts:103`); file does not exist.** `ignoreDeadLinks: false` → this is a build-breaking dead link. | Either restore the page (scoped to the *surviving* vector stores, since upstash is gone) or remove the sidebar entry. Likely **remove the entry** unless other vector adapters still ship — confirm at edit time. | R2 |
| `packages/memofs/connectors.md` | ADR 0002 + Q1–Q3 lock connectors (local execution, `connectors.json` 11th canonical file, secret handling) but **no docs page exists.** | **Create** — local execution model, config sync, `secretRef` (never the token), connector-write determinism. Single home; link from sidebar. Do NOT add an `api/memofs/connectors` page until `@memofs/connectors` package exists (no vapor docs). | R2/R3 |
| `packages/memofs/intelligence.md` | Q5 / ADR 0004 lock v1 intelligence (hybrid recall + LLM extraction + consolidation) but the "how smart is Memo FS" story is scattered/absent. | **Create** — projects the Q5 decision table (strong retrieval, rule-based+LLM graph, consolidation). | R3 |
| `packages/memofs/ai-sdk/*` adapter pointer | the AI SDK guide pages still live under `packages/memofs/ai-sdk/` but the code moved to an adapter package. | **Resolve routing:** keep the guide URL (don't break inbound links) but repoint all imports/exports at `@memofs/adapter-ai-sdk`. | R2 |
...
- Every "stale" claim cross-checked against landed code (D4 types in
  `types.ts`/`config.ts`; S2-Q1 extraction in `packages/memofs-adapter-ai-sdk/`;
...
- Dead-link detection run against `sidebar.mts` / `nav.mts` → the only genuine
  dead internal link is `/api/memofs/vector-adapters` (`/changelog/` is a
...
- **Product-claim preservation verified:** no entry in this register retracts,
  defers, or reframes Memo FS Cloud, the hosted MCP endpoint, or the
