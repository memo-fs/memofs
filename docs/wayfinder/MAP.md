...
2. **Naming map (reconciled with K4 — the core/cli cycle fix, with `tekmemo`
   ...
   |---|---|---|
   | `packages/client` | `@memofs/core` | **RENAMED** — it *is* the core runtime. Holds the primitives (K4). |
   | `packages/tekmemo` | `tekmemo` (unscoped) | **STAYS** — it is the CLI; `npm install -g tekmemo` is the expected install. Depends on `@memofs/core`. |
   | `packages/adapter-r2` | `@memofs/adapter-r2` | scope flip; **blob-only** (S3-Q3 decoupling) |
   | `packages/adapter-turso` *(new)* | `@memofs/adapter-turso` | **NEW** — Turso metadata extracted from the bundled R2 package (S3-Q3) |
   | `packages/adapter-openai` | `@memofs/adapter-openai` | scope flip |
   | `packages/adapter-voyage` | `@memofs/adapter-voyage` | scope flip |
   | `packages/adapter-transformers` | `@memofs/adapter-transformers` | scope flip |
   | `packages/adapter-workers-ai` | `@memofs/adapter-workers-ai` | scope flip |
   | `packages/adapter-ai-sdk` | `@memofs/adapter-ai-sdk` | scope flip |
   | `packages/connectors` | `@memofs/connectors` | scope flip |
   | `packages/json-rpc` | `@memofs/json-rpc` | scope flip |
   | `packages/server` | `@memofs/server` | scope flip — **load-bearing** (K1; slice 0 landed) |
   | `packages/mcp-server` | `@memofs/mcp-server` | scope flip |
   | `packages/benchmark-kit` | `@memofs/benchmark-kit` | scope flip |
   | `packages/testing` | `@memofs/testing` | scope flip |
   | `apps/cloud` | `@memofs/cloud` | private app — the rebuilt cloud (RRv8 + Hono + CF skeleton) |
   | `apps/docs` | `@memofs/docs` | private app |
   ...
   (`packages/client` → `packages/core`, renamed to `@memofs/core`).
...
6. **Storage adapters (K1 + S3-Q3):** **decoupled** — `@memofs/adapter-r2`
   (blob-only) + `@memofs/adapter-turso` (metadata-only), N+M packages not N×M.
...
- [WF-1 — Blob/metadata-decoupled adapter contract](tickets/WF-1-adapter-cloudflare-contract.md) — `@memofs/adapter-r2` (blob-only) + `@memofs/adapter-turso` (metadata-only) ship as decoupled packages over the provider-neutral `BlobClient`/`MetadataStore` interfaces in core (S3-Q3, carried under K1).
- [WF-5 — Amend ADRs + SSOT docs to match the overhaul](tickets/WF-5-adr-and-ssot-doc-amendments.md) — ADRs 0005/0011/0012/0013 amended for K1–K3; new [ADR 0016](../adr/0016-scope-flip-and-dir-equals-name.md) records the scope flip + K4 + dir-equals-name SSOT; decisions.md / CONTEXT.md / D2 doc + root docs flipped to `@memofs/*` and `packages/core`. K5 (GitHub Issues migration) + local-ticket deletion deferred.
