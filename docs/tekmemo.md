# Memo FS Core Package Notes (`@memofs/core`)
...
Memo FS ships as multiple focused packages under the `@memofs/` scope (see
...
- **Provider Adapters**: Provider adapters (such as OpenAI, VoyageAI, and Voyage
  Rerank) translate Memo FS contracts to provider APIs. They live in their own
...
A Memo FS capability is not release-ready until:
