...
const recall = await memo.recall({
  query: "database migrations",
  limit: 5,
});

// 5. Create snapshots and diffs
const snapshot = await memo.snapshot({ label: "before-refactor" });
const diff = await memo.diff("before-refactor", "after-refactor");

// 6. Maintenance & consolidation
await memo.consolidate({ apply: true });
await memo.archiveDeprecated();
await memo.restore("2026-08-10T11:21:09.163Z");
```
...
```
CliError (Base error)
├── CliUsageError (Invalid flags, missing arguments)
├── CliValidationError (Schema or format validation error)
├── CliFsError (Filesystem operation or traversal guard violation)
├── CliProtocolError (Protocol file or manifest corruption)
└── CliJsonlError (JSONL syntax error in strict mode)
```
