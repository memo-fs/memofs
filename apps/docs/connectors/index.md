---
title: "@memofs/connectors Framework"
...
```
┌─────────────────────────┐
│ .memofs/connectors.json │ ◄── Opaque secretRef (NO raw tokens)
└───────────┬─────────────┘
            │
            ▼
┌─────────────────────────┐      ┌──────────────────────────────┐
│  runConnectors(opts)    │ ◄────┤   SecretResolver             │
│  (Local Engine Runtime) │      │   • EnvSecretResolver        │
└───────────┬─────────────┘      │   • CloudSecretResolver      │
            │                    │   • StaticSecretResolver     │
            ▼                    └──────────────────────────────┘
┌───────────────────────────────────────────────────────────────┐
│ Host MemoFS Instance (Single-Writer Discipline)               │
│ • Deduplicates by deterministic ID (conn_<sha256[:16]>)       │
│ • Appends to .memofs/memory/notes.md (source: "connector")   │
│ • Emits memory.created events & updates recall indexes        │
└───────────────────────────────────────────────────────────────┘
```
...
### Configuration Schema (`ConnectorConfig`)
...
### 1. `EnvSecretResolver` (Local Development)
...
> [!NOTE]
> `EnvSecretResolver` reads from `.memofs/secrets.json` on disk (cached in memory); it does not read OS environment variables (`process.env`). Always ensure `.memofs/secrets.json` is listed in your project's `.gitignore`.

### 2. `CloudSecretResolver` (Production & Hosted CLI)
...
### 3. `StaticSecretResolver` (Testing & Embedding)
