...
```
┌────────────────────────────────────────────────────────┐
│ 1. Constructor Options (MemoFsConfig) - Highest        │
├────────────────────────────────────────────────────────┤
│ 2. Environment Variables (process.env)                 │
├────────────────────────────────────────────────────────┤
│ 3. Workspace Config File (.memofs/config.json)         │
├────────────────────────────────────────────────────────┤
...
## Node Filesystem Options (`NodeFsMemoryStoreOptions`)
...
```json
{
  "$schema": "https://memofs.dev/schema/config.json",
  "runtime": "local",
...
| `MEMOFS_API_KEY` | `string` | `undefined` | API key (`tm_...`) used to authenticate with MemoFS Cloud. |
| `MEMOFS_RECALL_ENGINE` | `string` | `"auto"` | Recall engine strategy: `"lexical"`, `"vector"`, `"hybrid"`, or `"auto"`. |
| `MEMOFS_LOCAL_EMBEDDINGS` | `string` | `"false"` | Set to `"1"` or `"true"` to enable local ONNX embeddings via Transformers.js. |
