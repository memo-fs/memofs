...
`@memofs/core` includes an integrated, versioned Knowledge Graph engine stored in `.memofs/graph/nodes.jsonl` and `.memofs/graph/edges.jsonl`. It models structured relationships between concepts, code symbols, tools, and decisions.
...
### 2. Exploring Neighbors (`memofs.graph.neighbors`)
...
### 3. Pathfinding & Dependency Tracing (`memofs.graph.path`)
...
The rule-based extractor recognizes 7 structural prose patterns:
1. **Definitions & Roles:** `"X is a Y"` → `X (concept) -> is_a -> Y`.
2. **Dependencies:** `"X depends on Y"`, `"X requires Y"` → `X -> depends_on -> Y`.
3. **Usage:** `"X uses Y"`, `"X connects to Y"` → `X -> uses -> Y`.
4. **Decisions:** `"We decided on X"`, `"Chosen X for Y"` → `X (decision) -> decided -> Y`.
5. **Authorship:** `"X was written by Y"` → `X -> authored_by -> Y`.
6. **Preferences:** `"Prefer X over Y"` → `X -> prefers -> Y`.
7. **Supersession & Migration:** `"Migrated from X to Y"`, `"Replaced X with Y"` → `Y -> supersedes -> X`.

## Memory Consolidation (`memofs.consolidate`)
