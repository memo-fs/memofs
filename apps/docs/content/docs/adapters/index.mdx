...
flowchart TD
    ...
    Storage["Storage Axis\n(RemoteBlobMemoryStore)"]
    Intelligence["Intelligence Axis\n(Embedders • Rerankers • Extractors)"]

    R2["@memofs/adapter-r2\n(Cloudflare R2)"]
    Turso["@memofs/adapter-turso\n(Turso / libSQL)"]

    OpenAI["@memofs/adapter-openai"]
    Voyage["@memofs/adapter-voyage"]
    Transformers["@memofs/adapter-transformers"]
    WorkersAI["@memofs/adapter-workers-ai"]
    AiSdk["@memofs/adapter-ai-sdk"]

    MF --> Storage & Intelligence
    Storage --> R2 & Turso
    Intelligence --> OpenAI & Voyage & Transformers & WorkersAI & AiSdk
```
...
| **[`@memofs/adapter-transformers`](/adapters/transformers)** | Intelligence | `MemoryEmbedder` | Node.js (>= 22) | 100% offline, local ONNX sentence embeddings (`Xenova/bge-small-en-v1.5`) with zero API keys or cloud dependencies. |
