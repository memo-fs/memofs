...
const memo = createMemoFSFromCli({
  root: "./my-project",
  runtime: "hybrid",
  cloudUrl: "https://cloud.memofs.dev/api/v1",
...
Returns a portable relative path (`../node_modules/@memofs/cli/schema/config.json`) if the package schema exists on disk, or the hosted fallback URL (`https://memofs.dev/schema/config.json`).
