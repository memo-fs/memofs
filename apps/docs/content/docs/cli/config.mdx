...
```bash
memofs config init
memofs config init --runtime hybrid --cloud-url https://cloud.memofs.dev/api/v1 --project-id my-project
...
2. Otherwise, falls back to the hosted URL `https://memofs.dev/schema/config.json`.
