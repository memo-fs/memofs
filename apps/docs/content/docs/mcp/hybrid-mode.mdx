  ...
  "mcpServers": {
    "memofs": {
      ...
      "args": [
        ...
        "@memofs/mcp-server",
        "--runtime", "hybrid",
        "--cloud-url", "https://cloud.memofs.dev/api/v1"
      ],
      "env": {
...
| `--cloud-url <url>` | **Yes** | MemoFS Cloud API root, e.g. `https://cloud.memofs.dev/api/v1`. |
...
API keys are either **read-write** or **read-only** (managed on the [dashboard's API Keys](https://cloud.memofs.dev/dashboard/api-keys) page). Each scope enables a different subset of the four memory verbs:
