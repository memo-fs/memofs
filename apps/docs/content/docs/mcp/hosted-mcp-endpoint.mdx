...
```
https://cloud.memofs.dev/api/v1/projects/<project-id>/mcp
```
...
The hosted endpoint speaks standard MCP streamable-HTTP, so any client that supports that transport works: Coding agents, hosted serverless agents, agents running in CI, and others. Config shapes are mostly interchangeable.


```sh
claude mcp add --transport http memofs https://cloud.memofs.dev/api/v1/projects/<project-id>/mcp \
...
// Save as `.cursor/mcp.json` in your project (or merge into the global one)
{
  "mcpServers": {
    "memofs": {
      "url": "https://cloud.memofs.dev/api/v1/projects/<project-id>/mcp",
      "headers": {
