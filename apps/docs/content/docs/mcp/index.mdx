...
```
  ...
  <Tab value="deno">
```bash
deno add npm:@memofs/mcp-server
```
  </Tab>
</Tabs>
...
```json title=".mcp.json (Claude Code / Desktop)"
// Project: ./.mcp.json
...
```toml title=".codex/config.toml (Codex)"
...
```jsonc title="opencode.jsonc (opencode)"
...
```json title=".agents/mcp_settings.json (Google Antigravity)"
...
```json title=".gemini/settings.json (Gemini CLI)"
...
| Tool Name | Safety | Description | Key Parameters |
|---|---|---|---|
| **`memofs_agent_session_start`** | `write` | Initializes an isolated agent session workspace under `agent-sessions/<id>/`. | `task` (required), `actorId`, `sessionId`, `workspaceId`, `projectId` |
| **`memofs_agent_session_read`** | `read` | Reads a file from the active agent session workspace (`context/`, `working/`, `output/`). | `sessionId` (required), `path` (required) |
| **`memofs_agent_session_write`** | `write` | Overwrites a file in the session working directory. | `sessionId` (required), `path` (required), `content` (required) |
| **`memofs_agent_session_append`** | `write` | Appends content to a file in the session working directory. | `sessionId` (required), `path` (required), `content` (required) |
| **`memofs_agent_session_extract`** | `read` | Extracts candidate durable takeaways from session output files. | `sessionId` (required), `workspaceId`, `projectId` |
| **`memofs_agent_session_complete`** | `write` | Finalizes session, optionally promoting durable memory to `notes.md` and taking a snapshot. | `sessionId` (required), `outcome` (`"success"` \| `"failure"` \| `"aborted"`), `extractDurableMemory`, `checkpointLabel`, `ephemeral`, `reason` |
