...
In your project root, store your Linear API key in `.memofs/secrets.json`:

```json title=".memofs/secrets.json"
{
...
```bash
npx @memofs/cli context --query "ENG-142 checkout refactor requirements"
```
