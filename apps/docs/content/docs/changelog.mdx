...
<Callout title="Per-Package Changelogs" type="info">
Each MemoFS package maintains its own changelog. This page highlights notable changes across the project.

**Per-package changelogs** — see each package's `CHANGELOG.md` for the full history:
[`@memofs/core`](https://github.com/memo-fs/memofs/blob/main/packages/core/CHANGELOG.md) ·
[`@memofs/cli`](https://github.com/memo-fs/memofs/blob/main/packages/cli/CHANGELOG.md) ·
[`@memofs/mcp-server`](https://github.com/memo-fs/memofs/blob/main/packages/mcp-server/CHANGELOG.md) ·
[`@memofs/server`](https://github.com/memo-fs/memofs/blob/main/packages/server/CHANGELOG.md) ·
[all packages →](https://github.com/memo-fs/memofs/releases)

**GitHub Releases** — [memo-fs/memofs/releases](https://github.com/memo-fs/memofs/releases) for tagged releases with notes.
</Callout>
...
#### Added
- Improved hybrid recall to merge duplicate hits and reward double matches, with single candidate per memory for reranking
- Added one-time cleanup for legacy embeddings on first load
- Switched default local embedding model to a stronger 384-dimension variant
- Added instruction-aware embedding and cross-model safety for queries

### CLI

#### Added
- Added validation schema for connectors configuration
- Connectors commands now stamp editor schema reference on every write
- Init now predownloads the embedding model and sets hybrid recall defaults

### Adapter Transformers

#### Added
- New default model and quantized weights for smaller and faster downloads
- Added instruction prefix handling and shared model cache

### Spec

#### Added
- Added manifest schema and validator
