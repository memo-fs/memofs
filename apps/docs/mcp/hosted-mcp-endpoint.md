...
description: "Connecting AI agents to remote MemoFS Cloud Hosted MCP servers over HTTPS with SSE/Streamable HTTP."
---
...
- [CLI memory commands](/cli/memory) — `memofs cloud sync push` to provision a project's hosted replica; `memofs cloud sync pull` to reconcile local memory with cloud writes.