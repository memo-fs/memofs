...
description: "Hybrid MCP operation mode combining local disk storage with cloud replica synchronization."
---
...
- [CLI memory commands](/cli/memory) — `memofs cloud sync pull` / `memofs cloud sync status` to reconcile local memory with the cloud replica.