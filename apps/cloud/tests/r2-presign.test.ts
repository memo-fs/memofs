...
const CONFIG = {
	...
	bucket: "tekmemo-blobs",
} as const;

const SHA = "9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08";

function mockEnv(overrides: Partial<CloudWorkerEnv> = {}): CloudWorkerEnv {
	return {
		BLOBS: {} as unknown as CloudWorkerEnv["BLOBS"],
...
describe("presignConfigFromEnv", () => {
	...
	it("parses PRESIGN_TTL_SECONDS into a number", () => {
		const cfg = presignConfigFromEnv(mockEnv({ PRESIGN_TTL_SECONDS: "300" }));
		expect(cfg.ttlSeconds).toBe(300);
	});

	it("returns undefined ttl for blank/invalid PRESIGN_TTL_SECONDS", () => {
		expect(
			presignConfigFromEnv(mockEnv({ PRESIGN_TTL_SECONDS: "" })).ttlSeconds,
		).toBeUndefined();
		expect(
			presignConfigFromEnv(mockEnv({ PRESIGN_TTL_SECONDS: "garbage" }))
				.ttlSeconds,
		).toBeUndefined();
		expect(
			presignConfigFromEnv(mockEnv({ PRESIGN_TTL_SECONDS: "-5" })).ttlSeconds,
		).toBeUndefined();
	});
});
...
describe("presign", () => {
	it("builds a PUT URL against the R2 S3 host with the content-addressed key", async () => {
		...
		expect(url).toContain(
			`tekmemo-blobs.testacct.r2.cloudflarestorage.com/${SHA}`,
		);
	});
	...
	it("rewrites the host when publicBaseUrl is set", async () => {
		const url = await presign(
			{ ...CONFIG, publicBaseUrl: "https://cdn.tekmemo.dev" },
			SHA,
			"GET",
		);
		expect(url.startsWith("https://cdn.tekmemo.dev/")).toBe(true);
