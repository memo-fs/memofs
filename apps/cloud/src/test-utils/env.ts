...
/**
 * The in-memory blob store a {@link FakeR2Bucket} is built over. Keyed by the
 * sha256 content-address the sync handler writes (`r2_key === sha256`), so the
 * `verifyUploaded` read returns the exact bytes `put` stored.
 */
 ...
 */
export type TestEnvOverrides = Partial<CloudWorkerEnv>;
 ...
 * @returns a `CloudWorkerEnv` ready to hand to `app.fetch(request, env)`.
 */
export function createTestEnv(overrides: TestEnvOverrides = {}): CloudWorkerEnv {
