	...
	return (
		<div className="p-6">
			...
			<PlanPicker
				...
				teamsProductId={teamsProductId}
			/>
			<ManageSubscriptionCard accountId={accountId} />
		</div>
	);
}
...
function CurrentPlanCard({
	...
	const consolidationPercent = capPercent(
		usage.consolidationUsedToday,
		maxConsolidation,
	);
	const preWarmPercent = capPercent(usage.preWarmUsedToday, maxPreWarm);
			...
			<CardContent>
				<div className="grid grid-cols-1 gap-6 border-t border-border/40 pt-4 sm:grid-cols-2 lg:grid-cols-4">
					<div>
						...
						{storagePercent > 70 && (
							<p className="mt-1.5 flex items-center gap-1 text-xs text-primary">
								<ArrowUpRight className="h-3.5 w-3.5" /> Approaching storage cap
								— consider upgrading
							</p>
						)}
					</div>
					<div>
						...
						<Progress value={connectorsPercent} className="h-2 rounded-none" />
					</div>
					<div>
						<div className="mb-1.5 flex items-center justify-between text-xs text-muted-foreground">
							<span>Consolidation runs</span>
							<span className="font-mono">
								{usage.consolidationUsedToday} of {formatCap(maxConsolidation)}{" "}
								/ day
							</span>
						</div>
						<Progress
					...
					<div>
						<div className="mb-1.5 flex items-center justify-between text-xs text-muted-foreground">
							...
							<span className="font-mono">
								{usage.preWarmUsedToday} of {formatCap(maxPreWarm)} / day
							</span>
						</div>
...
function ManageSubscriptionCard({ accountId }: { accountId?: string }) {
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription className="text-xs">
					Invoices, payment method, and cancellation are managed in your portal.
				</CardDescription>
			</CardHeader>
