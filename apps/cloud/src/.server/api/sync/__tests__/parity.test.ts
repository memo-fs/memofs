 ...
 * contract: the published `@memofs/core/cloud-client` (the frozen surface
...
describe("client ↔ server wire parity (round-trip)", () => {
	it("health is anonymous and unwraps the { data, meta } envelope", async () => {
		...
		expect(result.name).toBe("memofs-cloud");
	});
	...
	it("round-trips a file set: push → upload → complete → pull → status", async () => {
		...
		const localManifest = {
			".memofs/core.md": coreSha,
			".memofs/notes.md": notesSha,
		};
		...
		const completeResult = await c.sync.complete({
			uploaded: [
				{ path: ".memofs/core.md", sha256: coreSha },
				{ path: ".memofs/notes.md", sha256: notesSha },
			],
		...
		expect(completeResult.manifest[".memofs/core.md"]?.sha256).toBe(coreSha);
		expect(completeResult.manifest[".memofs/core.md"]?.sizeBytes).toBe(
			coreContent.length,
		);
		expect(completeResult.manifest[".memofs/notes.md"]?.sha256).toBe(notesSha);
		...
		expect(status.manifest[".memofs/core.md"]?.sha256).toBe(coreSha);
	...
	it("a second push with no changes asks the client to upload nothing", async () => {
		...
		await c.sync.push({
			manifest: { ".memofs/core.md": digest },
		});
		blobs.set(digest, {
		...
		await c.sync.complete({
			uploaded: [{ path: ".memofs/core.md", sha256: digest }],
			cursor: "0",
		});
	...
	it("pull reports a path the client holds but the cloud no longer has", async () => {
		...
		await c.sync.push({ manifest: { ".memofs/core.md": coreDigest } });
		blobs.set(coreDigest, {
			body: new TextEncoder().encode("core\n"),
			size: "core\n".length,
		});
		await c.sync.complete({
			uploaded: [{ path: ".memofs/core.md", sha256: coreDigest }],
			cursor: "0",
		});
		...
		expect(pulled.removed).toEqual([".memofs/stale.md"]);
	...
	it("complete surfaces a typed ConflictError when a blob is missing from R2", async () => {
		...
		await c.sync.push({ manifest: { ".memofs/core.md": claimed } });
	...
	it("complete surfaces a typed EntitlementError when storage exceeds the cap", async () => {
		...
		await c.sync.push({ manifest: { ".memofs/core.md": digest } });
		blobs.set(digest, {
