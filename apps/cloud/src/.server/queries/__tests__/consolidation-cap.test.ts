...
describe("consolidation cap enforcement (getAccountUsage + canRunConsolidation)", () => {
	...
	it("blocks a Pro run at exactly the 24/day cap (boundary)", async () => {
		...
		expect(canRunConsolidation("pro", usage.consolidationUsedToday)).toBe(
			false,
		);
	});
	...
	it("free tier: blocks after a single run (cap = 1)", async () => {
		...
		expect(canRunConsolidation("free", usage.consolidationUsedToday)).toBe(
			false,
		);
	});
	...
	it("teams tier: never blocks (Infinity cap)", async () => {
		...
		const usage = await getAccountUsage(ACCT);
		expect(canRunConsolidation("teams", usage.consolidationUsedToday)).toBe(
			true,
		);
	});
});
