...
/**
 * The most recent hosted-runtime events for a project, newest first (SC10).
 *
 * Backs the "Recent activity" feed alongside `recentSyncActivity`, surfacing the
 * *semantic* runtime events (consolidation runs, agent writes, core-memory
 * updates) rather than the byte-level replica commits. Cheap: one indexed scan
 * on `memory_events.project_id`, newest `created_at` first, `limit` rows.
 *
...
export async function recentMemoryActivity(
	...
	const rows = await db
		.select({
			...
			summary: memoryEvents.summary,
			actor: memoryEvents.actor,
			at: memoryEvents.createdAt,
		})
		.from(memoryEvents)
		.where(eq(memoryEvents.projectId, projectId))
