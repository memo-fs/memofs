 ...
 *     carrying `tekmemo_account_id` + `tekmemo_plan` is supplied by the dashboard
...
/** Read the plan tier from subscription metadata (`tekmemo_plan`), or null. */
function planFromMetadata(
	metadata: Record<string, unknown> | null | undefined,
): PlanTier | null {
	const raw = metadata?.tekmemo_plan;
	return isPlanMetadataValue(raw) ? raw : null;
}

/**
 * Resolve the TekMemo account for a subscription event, record the Polar
 * customer link, and apply the plan. Resolution order: `metadata.tekmemo_account_id`
...
async function handleSubscriptionEvent(
	...
	const metaAccountId =
		typeof event.data.metadata?.tekmemo_account_id === "string"
			? event.data.metadata.tekmemo_account_id
			: undefined;
	const account =
	...
	.post("/webhook", async (c) => {
		...
		switch (event.type) {
			...
			case "customer.created": {
				...
				const metaAccountId =
					typeof metadata?.tekmemo_account_id === "string"
						? metadata.tekmemo_account_id
						: undefined;
