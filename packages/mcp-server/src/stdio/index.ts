...
import type { MemoFSMcpProtocolServer } from "../protocol/server";
...
export async function runStdioServer(
	server: MemoFSMcpProtocolServer,
): Promise<void> {
	...
	for await (const line of rl) {
		...
		try {
			...
			stderr.write(
				`[memofs-mcp] fatal stdio error: ${error instanceof Error ? error.message : String(error)}\n`,
			);
		}
	}
}
