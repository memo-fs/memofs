 ...
 * `run` invokes the `@tekbreed/tekmemo-connectors` framework with the host's
 * `Tekmemo` instance (single-writer contract, AGENTS.md).
 *
 * @module commands/connectors
 */

import type { JsonObject, Tekmemo } from "@tekbreed/tekmemo";
...
export async function runConnectorsRunCommand(
	...
	const { runConnectors } = await import("@tekbreed/tekmemo-connectors");
