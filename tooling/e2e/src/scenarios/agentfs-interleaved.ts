...
export async function runAgentFsInterleavedScenario(
	...
	try {
		...
		const pathsAny = startRes.paths as unknown as {
			working?: { notes?: string; plan?: string; [k: string]: string | undefined };
		...
		if (!workingNotesPath.includes("/working/") && !workingNotesPath.includes("/output/")) {
			throw new Error(`workingNotesPath does not contain /working/ or /output/: ${workingNotesPath}`);
		}
		...
		const writeRes = await harness.client.agentfs.writeFile({
			...
			content: "# AgentFS E2E Interleaved\n\nSimba prefers TypeScript\n\nRUN_ID test-run-e2e-0021-agentfs-01\n",
		});
		details.writeRes = writeRes;

		// Step 3: Interleaved memory remember 1
		await harness.remember("AgentFS interleaved fact 1: Simba uses MemoFS for context injection RUN_ID test-run-e2e-0021-agentfs-02");
		...
		if (!readRes.content.includes("Simba")) {
			throw new Error(`read content missing Simba: ${readRes.content.slice(0, 200)}`);
		}
		if (!readRes.content.includes("Progress")) {
			throw new Error(`read content missing appended Progress: ${readRes.content.slice(0, 200)}`);
		}
		...
		const outputPath = (pathsAny?.output?.notes as string) ?? workingNotesPath.replace("/working/", "/output/").replace("notes.md", "result.md");
		const outputWritePath = outputPath.includes("/working/") || outputPath.includes("/output/") ? outputPath : workingNotesPath.replace("notes.md", "output.md").replace("/working/", "/output/");
		...
		const extractRes = await harness.client.agentfs.extract({ sessionId, projectId });
		details.extractKeys = Object.keys(extractRes as unknown as Record<string, unknown>);
		...
		await harness.remember("AgentFS final fact: MCP read-only guard blocks writes RUN_ID test-run-e2e-0021-agentfs-05");
		...
		if (searchAfter.length === 0) {
			...
			if (!all.includes("agentfs interleaved")) {
				throw new Error("search after interleaved returned 0 and snapshot missing fact");
			}
		}
		...
		details.complete = {
			...
			durableMemoryWritten: (completeRes as { durableMemoryWritten?: boolean }).durableMemoryWritten,
		};
		...
		details.filesContainingAgents = files.filter((f) => f.includes("agent") || f.includes("session")).slice(0, 10);
		details.totalFiles = files.length;

		if (!hasMemofsDir) throw new Error(".memofs/ missing after agentfs interleaved");
		if (!hasAgentFiles) throw new Error("agent files missing after scenario — expected .memofs/agents/ or similar");
