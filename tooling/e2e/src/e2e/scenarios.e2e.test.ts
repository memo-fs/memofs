...
describe("scenarios — lifecycle, agentfs-interleaved, connectors-merge, failure-recovery, golden-snapshots (ticket 65)", () => {
	it("lifecycle — CLI init → core 20 remembers → search paraphrase → context --json → consolidate preview+apply → asserts file count, manifest, events, no data loss, graph dedup", async () => {
		...
		expect(result.passed, `lifecycle failed: ${JSON.stringify(result.details).slice(0, 1000)}`).toBe(true);
		...
		expect((result.details as { searchResultsCount: number }).searchResultsCount).toBeGreaterThan(0);
		expect((result.details as { searchAfterCount: number }).searchAfterCount).toBeGreaterThan(0);

		const preview = (result.details as { consolidatePreview: { applied: boolean; mergesApplied: number } }).consolidatePreview;
		expect(preview.applied).toBe(false);
		expect(preview.mergesApplied).toBe(0);

		const applied = (result.details as { consolidateApplied: { applied: boolean } }).consolidateApplied;
		...
		const goldenCheck = await assertGoldenSnapshot("lifecycle", result.snapshot);
		expect(goldenCheck.goldenExists, "golden snapshot for lifecycle should exist (checked in)").toBe(true);
		expect(goldenCheck.mismatches.filter((m) => m.includes("required")).length, `golden mismatches: ${goldenCheck.mismatches.join("; ")}`).toBe(0);
	}, 60_000);
	...
	it("agentfs-interleaved — start/write/append/read/extract/complete + memory remember interleaved, session files under .memofs/agents/", async () => {
		...
		expect(result.passed, `agentfs-interleaved failed: ${JSON.stringify(result.details).slice(0, 1000)}`).toBe(true);
		expect(result.fileFirstTruth.hasMemofsDir).toBe(true);
		expect(result.fileFirstTruth.hasMemoryFiles).toBe(true);

		const files = result.snapshot.files;
		const hasAgentFiles = files.some((f) => f.includes("agents") || f.includes("agent-sessions") || f.includes(".memofs/agents"));
		...
		expect((result.details as { readContentLength: number }).readContentLength).toBeGreaterThan(0);

		// Golden
		const goldenCheck = await assertGoldenSnapshot("agentfs-interleaved", result.snapshot);
		expect(goldenCheck.goldenExists).toBe(true);
		expect(goldenCheck.mismatches.filter((m) => m.includes("required")).length).toBe(0);
	}, 60_000);
	...
	it("connectors-merge — MSW GitHub/Notion ingest + local memory merge + dedup + source attribution, second run idempotent", async () => {
		...
		expect(result.passed, `connectors-merge failed: ${JSON.stringify(result.details).slice(0, 1000)}`).toBe(true);
		expect(result.fileFirstTruth.hasMemofsDir).toBe(true);
		expect(result.fileFirstTruth.hasManifest).toBe(true);

		const firstRun = (result.details as { firstRun: { written: number } }).firstRun;
		expect(firstRun.written).toBeGreaterThan(0);

		const secondRun = (result.details as { secondRun: { written: number; skipped: number } }).secondRun;
		...
		// Golden
		const goldenCheck = await assertGoldenSnapshot("connectors-merge", result.snapshot);
		expect(goldenCheck.goldenExists).toBe(true);
		expect(goldenCheck.mismatches.filter((m) => m.includes("required")).length).toBe(0);
	}, 60_000);
	...
	it("failure-recovery — kill mid-write, corrupt core.md, MSW 500/latency, doctor/validate recovery, no silent data loss", async () => {
		...
		expect(result.passed, `failure-recovery failed: ${JSON.stringify(result.details).slice(0, 1500)}`).toBe(true);
		expect(result.fileFirstTruth.hasMemofsDir).toBe(true);
		expect(result.fileFirstTruth.hasManifest).toBe(true);

		// No silent data loss: search after restore >0
		const searchAfterRestore = (result.details as { searchAfterRestore?: number }).searchAfterRestore;
		...
		// Golden
		const goldenCheck = await assertGoldenSnapshot("failure-recovery", result.snapshot);
		expect(goldenCheck.goldenExists).toBe(true);
	}, 120_000);
	...
	it("golden-snapshots — asserts exact .memofs/ layout after each scenario (file list + content hash), snapshot files checked in", async () => {
		...
		expect(result.passed, `golden-snapshots failed: ${JSON.stringify(result.details).slice(0, 1000)}`).toBe(true);
		expect(result.fileFirstTruth.hasMemofsDir).toBe(true);

		const goldenResults = (result.details as { goldenResults: Record<string, { exists: boolean; fileCount: number }> }).goldenResults;
		...
		// Self golden check
		const selfCheck = (result.details as { selfGoldenCheck: { passed: boolean; goldenExists: boolean } }).selfGoldenCheck;
		...
		expect(Object.keys(result.snapshot.hashes).length).toBeGreaterThanOrEqual(2);
	}, 30_000);
});
